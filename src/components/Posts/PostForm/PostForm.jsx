...
export default function PostForm() {
  ...
  const formEditPost = (edited_post) => {
    dispatch(editPost(edited_post));
  };

  const formAddPost = (new_post) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      if (id) {
        const edited_post = { id: id, ...values };
        formEditPost(edited_post);
      } else {
        const new_post = {
          id: new Date().valueOf(),
          ...values,
